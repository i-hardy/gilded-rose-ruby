...
describe BackstagePass do
  subject(:backstage_pass) do
    described_class.new("Backstage passes to a TAFKAL80ETC concert", 15, 10)
  end
  ...
  describe "#update_quality" do
    ...
    subject(:fab_backstage_pass) { described_class.new("Pass", 15, 50) }

    it "increases the quality by 1 when sell_in is greater than 10" do
      expect { backstage_pass.update_quality }
        .to change { backstage_pass.quality }
        .by GildedRose::SELL_IN_CHANGE
    end

    it "increases the quality by 2 when sell_in is between 5 and 10" do
      expect { nearer_backstage_pass.update_quality }
        .to change { nearer_backstage_pass.quality }
        .by GildedRose::SELL_IN_CHANGE * 2
    end

    it "increases the quality by 3 when sell_in is less than 5" do
      expect { nearest_backstage_pass.update_quality }
        .to change { nearest_backstage_pass.quality }
        .by GildedRose::SELL_IN_CHANGE * 3
    end
    ...
    it "cannot increase the quality beyond 50" do
      expect { fab_backstage_pass.update_quality }
        .not_to change { fab_backstage_pass.quality }
    end
  end

  describe "#to_s" do
    it "returns a string with the item name and its sell_in and quality" do
      expect(backstage_pass.to_s)
        .to eq "Backstage passes to a TAFKAL80ETC concert, 15, 10"
    end
  end
end
